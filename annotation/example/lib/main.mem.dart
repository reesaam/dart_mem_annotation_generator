// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'main.dart';

// **************************************************************************
// Generator: AnnotationBuilder
// **************************************************************************

///   This File is Generated by Dart Mem Annotation Generator
///   [Sample] Annotated with [Mem] with [all] mode
///   Including:
///     Model Class, Entity Class and Mapper Class,
///     Model Classes includes [toJson] and [fromJson] functions, and Entity Classes are not!
///     Mappers will convert every filed in the class including [SubClasses], they should be annotated as well.
///     [Mem] Annotation will generate all these fields for all Classes, so all subclasses should decorated with annotation to generate model, entity and mapper to use them here
///     [Enums] also supported and they will be detected and count into account for mappers and also json conversion

///   Details:
///     Class: Sample
///     Constructors Count: 1
///     Variables Count: 6

///   ==> Model Class:
class SampleModel {
  SampleModel({
    required this.sampleEnum,
    required this.name,
    required this.description,
    required this.innerClassModel,
    required this.sampleList,
    required this.innerSampleListModel,
  });

  SampleEnum? sampleEnum;
  String? name;
  String? description;
  InnerSampleModel? innerClassModel;
  List<String>? sampleList;
  List<InnerSampleModel>? innerSampleListModel;

  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{};
    map['sampleEnum'] = sampleEnum?.name;
    map['name'] = name;
    map['description'] = description;
    map['innerClassModel'] = innerClassModel?.toJson();
    map['sampleList'] = sampleList;
    map['innerSampleListModel'] =
        innerSampleListModel?.map((e) => e.toJson()).toList();
    return map;
  }

  SampleModel.fromJson(dynamic json) {
    sampleEnum =
        SampleEnum.values.firstWhere((e) => e.name == json['sampleEnum']);
    name = json[name];
    description = json[description];
    innerClassModel = InnerSampleModel.fromJson(json['innerClassModel']);
    sampleList = json[sampleList];
    innerSampleListModel = (json[innerSampleListModel] as List)
        .map((e) => InnerSampleModel.fromJson(e))
        .toList();
  }
}

///   ==> Entity Class:
class SampleEntity {
  SampleEntity({
    required this.sampleEnum,
    required this.name,
    required this.description,
    required this.innerClassEntity,
    required this.sampleList,
    required this.innerSampleListEntity,
  });

  SampleEnum? sampleEnum;
  String? name;
  String? description;
  InnerSampleEntity? innerClassEntity;
  List<String>? sampleList;
  List<InnerSampleEntity>? innerSampleListEntity;
}

///   ==> Mapper Classes:
///   ==> Entity to Model Mapper Extension
extension MapperExtensionOnSampleEntity on SampleEntity {
  SampleModel get mapper => SampleModel(
        sampleEnum: sampleEnum,
        name: name,
        description: description,
        innerClassModel: innerClassEntity?.mapper,
        sampleList: sampleList,
        innerSampleListModel:
            innerSampleListEntity?.map((e) => e.mapper).toList(),
      );
}

///   ==> Model to Entity Mapper Extension
extension MapperExtensionOnSampleModel on SampleModel {
  SampleEntity get mapper => SampleEntity(
        sampleEnum: sampleEnum,
        name: name,
        description: description,
        innerClassEntity: innerClassModel?.mapper,
        sampleList: sampleList,
        innerSampleListEntity:
            innerSampleListModel?.map((e) => e.mapper).toList(),
      );
}

///   This File is Generated by Dart Mem Annotation Generator
///   [InnerSample] Annotated with [Mem] with [all] mode
///   Including:
///     Model Class, Entity Class and Mapper Class,
///     Model Classes includes [toJson] and [fromJson] functions, and Entity Classes are not!
///     Mappers will convert every filed in the class including [SubClasses], they should be annotated as well.
///     [Mem] Annotation will generate all these fields for all Classes, so all subclasses should decorated with annotation to generate model, entity and mapper to use them here
///     [Enums] also supported and they will be detected and count into account for mappers and also json conversion
///     Classes are decorated with [Freezed], other functions will generate with [Freezed]

///   Details:
///     Class: InnerSample
///     Constructors Count: 1
///     Variables Count: 2

///   ==> Model Class:
@Freezed(toJson: true, fromJson: true)
abstract class InnerSampleModel with _$InnerSampleModel {
  const factory InnerSampleModel({
    final String? name,
    final String? description,
  }) = _InnerSampleModel;

  factory InnerSampleModel.fromJson(Map<String, dynamic> json) =>
      _$InnerSampleModelFromJson(json);
}

///   ==> Entity Class:
@Freezed(toJson: false, fromJson: false)
abstract class InnerSampleEntity with _$InnerSampleEntity {
  const factory InnerSampleEntity({
    final String? name,
    final String? description,
  }) = _InnerSampleEntity;
}

///   ==> Mapper Classes:
///   ==> Entity to Model Mapper Extension
extension MapperExtensionOnInnerSampleEntity on InnerSampleEntity {
  InnerSampleModel get mapper => InnerSampleModel(
        name: name,
        description: description,
      );
}

///   ==> Model to Entity Mapper Extension
extension MapperExtensionOnInnerSampleModel on InnerSampleModel {
  InnerSampleEntity get mapper => InnerSampleEntity(
        name: name,
        description: description,
      );
}

///   This File is Generated by Dart Mem Annotation Generator
///   [SampleMEMModel] Annotated with [Mem] with [model] mode
///   Including:
///     Model Class, Entity Class and Mapper Class,
///     Model Classes includes [toJson] and [fromJson] functions, and Entity Classes are not!
///     Mappers will convert every filed in the class including [SubClasses], they should be annotated as well.
///     [Mem] Annotation will generate all these fields for all Classes, so all subclasses should decorated with annotation to generate model, entity and mapper to use them here
///     [Enums] also supported and they will be detected and count into account for mappers and also json conversion

///   Details:
///     Class: SampleMEMModel
///     Constructors Count: 1
///     Variables Count: 3

///   ==> Model Class:
class SampleMEMModelModel {
  SampleMEMModelModel({
    required this.name,
    required this.description,
    required this.sampleList,
  });

  String? name;
  String? description;
  List<String>? sampleList;

  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{};
    map['name'] = name;
    map['description'] = description;
    map['sampleList'] = sampleList;
    return map;
  }

  SampleMEMModelModel.fromJson(dynamic json) {
    name = json[name];
    description = json[description];
    sampleList = json[sampleList];
  }
}

///   This File is Generated by Dart Mem Annotation Generator
///   [SampleMEMModelFreezed] Annotated with [Mem] with [model] mode
///   Including:
///     Model Class, Entity Class and Mapper Class,
///     Model Classes includes [toJson] and [fromJson] functions, and Entity Classes are not!
///     Mappers will convert every filed in the class including [SubClasses], they should be annotated as well.
///     [Mem] Annotation will generate all these fields for all Classes, so all subclasses should decorated with annotation to generate model, entity and mapper to use them here
///     [Enums] also supported and they will be detected and count into account for mappers and also json conversion
///     Classes are decorated with [Freezed], other functions will generate with [Freezed]

///   Details:
///     Class: SampleMEMModelFreezed
///     Constructors Count: 1
///     Variables Count: 3

///   ==> Model Class:
@Freezed(toJson: true, fromJson: true)
abstract class SampleMEMModelFreezedModel with _$SampleMEMModelFreezedModel {
  const factory SampleMEMModelFreezedModel({
    final String? name,
    final String? description,
    final List<String>? sampleList,
  }) = _SampleMEMModelFreezedModel;

  factory SampleMEMModelFreezedModel.fromJson(Map<String, dynamic> json) =>
      _$SampleMEMModelFreezedModelFromJson(json);
}

///   This File is Generated by Dart Mem Annotation Generator
///   [SampleMEMEntity] Annotated with [Mem] with [entity] mode
///   Including:
///     Model Class, Entity Class and Mapper Class,
///     Model Classes includes [toJson] and [fromJson] functions, and Entity Classes are not!
///     Mappers will convert every filed in the class including [SubClasses], they should be annotated as well.
///     [Mem] Annotation will generate all these fields for all Classes, so all subclasses should decorated with annotation to generate model, entity and mapper to use them here
///     [Enums] also supported and they will be detected and count into account for mappers and also json conversion

///   Details:
///     Class: SampleMEMEntity
///     Constructors Count: 1
///     Variables Count: 3

///   ==> Entity Class:
class SampleMEMEntityEntity {
  SampleMEMEntityEntity({
    required this.name,
    required this.description,
    required this.sampleList,
  });

  String? name;
  String? description;
  List<String>? sampleList;
}

///   This File is Generated by Dart Mem Annotation Generator
///   [SampleMEMEntityFreezed] Annotated with [Mem] with [entity] mode
///   Including:
///     Model Class, Entity Class and Mapper Class,
///     Model Classes includes [toJson] and [fromJson] functions, and Entity Classes are not!
///     Mappers will convert every filed in the class including [SubClasses], they should be annotated as well.
///     [Mem] Annotation will generate all these fields for all Classes, so all subclasses should decorated with annotation to generate model, entity and mapper to use them here
///     [Enums] also supported and they will be detected and count into account for mappers and also json conversion
///     Classes are decorated with [Freezed], other functions will generate with [Freezed]

///   Details:
///     Class: SampleMEMEntityFreezed
///     Constructors Count: 1
///     Variables Count: 3

///   ==> Entity Class:
@Freezed(toJson: false, fromJson: false)
abstract class SampleMEMEntityFreezedEntity
    with _$SampleMEMEntityFreezedEntity {
  const factory SampleMEMEntityFreezedEntity({
    final String? name,
    final String? description,
    final List<String>? sampleList,
  }) = _SampleMEMEntityFreezedEntity;
}
